...
from leetcode_cli.services.config_service import (
    get_config_path,
    get_config_dir,
    load_config,
    save_config
...
def initialize_leetcode_cli():
    ...
    try:
        ...
        for theme_name in available_themes:
            _ensure_theme_folder(theme_name)
...
def _ensure_minimum_config_fields():
    """
    Fills ~/.config/leetcode/config.json with default values if they’re missing.
    """
    try:
        config = load_config()
        updated = False
        ...
        logger.error(f"Error ensuring minimum config fields: {e}")
        raise
...
def _ensure_theme_folder(theme_name: str):
    """
