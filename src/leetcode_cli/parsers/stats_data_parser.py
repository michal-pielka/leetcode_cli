...
def parse_user_stats_data(json_data: dict[str, Any]) -> UserStatsModel:
    ...
    try:
        ...
        untouched = {
            item["difficulty"].upper(): item["count"] for item in user_progress.get("numUntouchedQuestions", [])
        }
        ...
        raise ParsingError(f"Missing key in stats data: {e}") from e
    except TypeError as e:
        logger.error(f"Invalid structure in stats data: {e}")
        raise ParsingError(f"Invalid structure in stats data: {e}") from e
...
def parse_single_year_calendar(json_data: dict[str, Any]) -> dict[int, int]:
    ...
    try:
        ...
        raise ParsingError(f"Missing or invalid calendar data: {e}") from e
    ...
    try:
        ...
        raise ParsingError(f"Invalid submissionCalendar JSON: {e}") from e
