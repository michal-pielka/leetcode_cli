...
from .user_utils import get_problem_by_key_value, set_chosen_problem, get_chosen_problem, get_random_problem, get_problems_data_path
...
POSSIBLE_FILE_EXTENTIONS = ["cpp", "java", "py", "py3", "c", "cs", "js", "ts", "php", "swift", "kt", "dart", "go", "rb", "scala", "rs", "rkt", "erl", "ex"]
...
extension_to_lang_slug = {
    POSSIBLE_FILE_EXTENTIONS[i] : POSSIBLE_LANG_SLUGS[i] for i in range(len(POSSIBLE_LANG_SLUGS))
}

lang_slug_to_extension = {
    POSSIBLE_LANG_SLUGS[i] : POSSIBLE_FILE_EXTENTIONS[i] for i in range(len(POSSIBLE_FILE_EXTENTIONS))
}
...
def config(key, value):
    ...
    if key == 'cookie':
        ...
        click.echo(f"sername set to {ANSI_CODES["ITALIC"]}{value}.{ANSI_RESET}")
...
@click.option(
    '--tag',
    multiple=True,
    type=click.Choice(["all"] + POSSIBLE_TAGS, case_sensitive=False),
    default=('all',), 
    show_default=True,
    help='Filter by tag (default: all)'
)
@click.option(
    '--limit',
    type=int,
...
def list(difficulty, tag, limit, page):
...
@click.option(
    ...
    multiple=True,
    type=click.Choice([
        "all", "title", "tag", "langs", "description",
        "examples", "constraints"
    ], case_sensitive=False),
    help='Sections to display (default: all). Options: title, tag, langs, description, examples, constraints'
)
@click.option(
    '--random',
    is_flag=True,
...
def show(title_slug_or_id, include, random, difficulty, tag):
    """
    ...
    try:
        ...
        if (difficulty or tag) and not random:
            click.echo("Error: '--difficulty' and '--tag' options can only be used with '--random'.")
            return

        if random:
            problems_metadata_path = get_problems_data_path()
            if not os.path.exists(problems_metadata_path):
                click.echo("Error: 'problems_metadata.json' file not found. Please download it first using 'leetcode download-problems'.")
                return

            selected_problem = get_random_problem(difficulty=difficulty, tags=tag)
        ...
        if title_slug_or_id.isdigit():
            ...
            if not problem_data:
                click.echo(f"Error: In order to fetch problem data by question ID, you need to download problems' metadata using 'leetcode download-problems'.")
                return
        else:
...
@click.option(
    '--include',
    multiple=True,
    type=click.Choice(['all', 'stats', 'calendar'], case_sensitive=False),
    default=("all",),  # Correctly set as a tuple
    show_default=True,
...
def stats(username, include):
    ...
    try:
        ...
        normalized_include = [section.lower() for section in include]

        if 'all' in normalized_include:
            sections_to_include = VALID_SECTIONS
        else:
            sections_to_include = normalized_include
